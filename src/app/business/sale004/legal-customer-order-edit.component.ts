...
@Component({
  selector: 'app-sale004-legal-customer-order-edit',
  standalone: true,
      ...
      <div class="scrollable-panel">
        <ng-container>
          ...
          <nz-card [nzBordered]="false" nzTitle="商品信息">
            <app-sale004-order-product-edit #t [items]="i.items" (itemsChange)="i.items = $event" />
          ...
          <nz-card [nzBordered]="false" nzTitle="付款信息">
            <app-sale004-order-payment-edit #p [items]="i.payments" (itemsChange)="i.payments = $event" />
...
export class LegalCustomerOrderEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.i.id > 0) {
      this.http.get(`/sale004/legal-customer-order/load/${this.i.id}`).subscribe(res => (this.i = res.list[0]));
    }
  }

  save(): void {
    this.http.post(`/sale004/legal-customer-order/save/${this.i.id}`, this.i).subscribe(() => {
