...
@Component({
  selector: 'app-sale003-order-payment-edit',
  standalone: true,
            ...
            <nz-form-item *ngIf="editObjs.has(item)">
              <nz-form-control nzErrorTip="请选择状态">
                <nz-select [(ngModel)]="items[i]!.status" placeholder="请选择状态">
                  <nz-option *ngFor="let status of paymentStatusOptions" [nzValue]="status.value" [nzLabel]="status.label" />
                </nz-select>
              </nz-form-control>
            </nz-form-item>
          </td>
...
export class OrderPaymentEditComponent {
  @Input() items: Payment[] = [];
  ...
  add(): void {
    let newObj = { key: '', payment_id: '', payment_date: '', amount: 0, status: '1' };
