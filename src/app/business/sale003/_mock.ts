...
export const CORPORATE_CUSTOMER_ORDER = {
  '/sale003/corporate-customer-order/query': (req: MockRequest) => genData(req.queryString),
  '/sale003/corporate-customer-order/load/:id': (req: MockRequest) => list.find(w => w.id === +req.params.id),
  'POST /sale003/corporate-customer-order/save/:id': (req: MockRequest) => saveData(+req.params.id, req.body)
};
