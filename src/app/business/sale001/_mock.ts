...
export const GROUP_CUSTOMER_ORDER = {
  '/sale001/group-customer-order/query': (req: MockRequest) => genData(req.queryString),
  '/sale001/group-customer-order/load/:id': (req: MockRequest) => list.find(w => w.id === +req.params.id),
  'POST /sale001/group-customer-order/save/:id': (req: MockRequest) => saveData(+req.params.id, req.body)
};
