...
@Component({
  selector: 'app-sale001-group-customer-order-edit',
  standalone: true,
  imports: [SharedModule, OrderProductEditComponent, EmployeeEditComponent],
  template: `
    <div class="modal-header">
      <div class="modal-title">{{ i.id > 0 ? '编辑' : '添加' }}-团体客户仓订单</div>
    </div>
      ...
      <div class="scrollable-panel">
        <ng-container>
          ...
          <nz-card [nzBordered]="false" nzTitle="商品信息">
            <app-sale001-order-product-edit #t [items]="i.items" (itemsChange)="i.items = $event" />
            <button nz-button type="button" [nzType]="'dashed'" (click)="t.add()" nzBlock class="mt-md">
              <i nz-icon nzType="plus"></i>
              <span>新增商品</span>
            </button>
          </nz-card>
          <nz-card [nzBordered]="false" nzTitle="雇员信息">
            <app-sale001-employee-edit #e [items]="i.employees" (itemsChange)="i.employees = $event" />
            <button nz-button type="button" [nzType]="'dashed'" (click)="e.add()" nzBlock class="mt-md">
              <i nz-icon nzType="plus"></i>
              <span>新增雇员</span>
            </button>
          </nz-card>
...
export class GroupCustomerOrderEditComponent implements OnInit {
